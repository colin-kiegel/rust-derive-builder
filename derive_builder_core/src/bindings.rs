...
impl Bindings {
    ...
    /// TryFrom trait.
    pub fn try_from_trait(&self) -> RawTokens<&'static str> {
        RawTokens(if self.no_std {
...
fn std() {
    ...
    assert_eq!(b.string_ty().to_tokens(), quote!(
        ::std::string::String
    ));

    assert_eq!(b.result_ty().to_tokens(), quote!(
        ::std::result::Result
    ));

    assert_eq!(b.option_ty().to_tokens(), quote!(
        ::std::option::Option
    ));

    assert_eq!(b.phantom_data_ty().to_tokens(), quote!(
        ::std::marker::PhantomData
    ));

    assert_eq!(b.default_trait().to_tokens(), quote!(
        ::std::default::Default
    ));

    assert_eq!(b.clone_trait().to_tokens(), quote!(
        ::std::clone::Clone
    ));

    assert_eq!(b.into_trait().to_tokens(), quote!(
        ::std::convert::Into
    ));
}
